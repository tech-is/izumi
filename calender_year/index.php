...
<?php
  ...
  if ($conf_horiday) {
    ...
    foreach ($pieces as $key => $value) {
      $temp = explode(',', $value);
      $csvyear = substr($temp[0] , 0 , 4) . "<br>";
      if($csvyear > 2019){
        $horiday[str_replace("/","-",$temp[0])] = $temp[1];  //2020年以降日付と祝日を設定
      }
    }
  }
for ($i = 0; $i <= 365; $i++) {
    ...
    //頭の0をなくす
    $chkday = date('Y-n-j', mktime(0, 0, 0, date('m'), date('1') + $i, date('Y')));
    //日付を格納する
    $days[$i]['day'] = $day;
    //祝日を設定する
    if ($conf_horiday) {
      $ind = array_key_exists($chkday,$horiday);
      if ($ind){
        //echo $chkday . "<br>";
        $days[$i]['hori'] = $horiday[$chkday];
      } else {
        $days[$i]['hori'] = '';
      }
    } else {
