...
class Bbs extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('bbs/bbs_view', $data);
        //$this->load->view('bbs/admin_view', $data);
        //$this->load->view('bbs/login_view');
				//$this->load->view('bbs/admin_edit_view');
				//$this->load->view('bbs/admin_delete_view');
    }
		...
		if (!empty($_SESSION['admin_login'])) {
			...
			if(!empty($id) && is_numeric($id)) {
					...
					if (!empty($data['message_data'])) {
						//$this->load->view('header_view');
						$this->load->view('bbs/admin_edit_view', $data);
					} else {
	...
	public function delete()
	{
		if (!empty($_SESSION['admin_login'])) {
			...
			if(!empty($id) && is_numeric($id)) {
					...
					if(!empty($data['message_data'])) {
					//$this->load->view('header_view');
					$this->load->view('bbs/admin_delete_view', $data);
					} else {
						$_SESSION['error_message'][] = '存在しないレコードです。';
						header('location: /bbs/admin');
					}
